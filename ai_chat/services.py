...
def _strip_legacy_customer_priority_ai_fields(analysis_result):
    if not isinstance(analysis_result, dict):
        return analysis_result
    for key in (
        ...
        'followup_priority_recommendation',
    ):
        analysis_result.pop(key, None)
    goals = analysis_result.get('recommended_goals')
    if isinstance(goals, list):
        cleaned_goals = []
        for goal in goals:
            if not isinstance(goal, dict) or goal.get('source') == 'priority_recommendation':
                continue
            goal.pop('priority', None)
            goal.pop('priorityLabel', None)
            goal.pop('priority_label', None)
            cleaned_goals.append(goal)
        analysis_result['recommended_goals'] = cleaned_goals[:10]
    return analysis_result


def apply_followup_priority_recommendations(analysis_result, followups, stage_contexts=None):
    """Deprecated: AI no longer reads or writes legacy customer priority."""
    _strip_legacy_customer_priority_ai_fields(analysis_result)
    return []
...
8. recommended_goals에 입력 데이터의 정확한 고객명이 들어갔는가?
"""
...
- next_best_actions은 실행 필요도가 높은 순으로 최대 3건 제시하고, 각 항목은 실제 영업
...
def analyze_followup(analysis, followup, user):
    ...
    parts.append(f"등급: {followup.customer_grade}")
