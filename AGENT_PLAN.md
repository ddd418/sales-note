...
- 구현/로컬 검증 완료. `EmailLog`의 `followup`/`schedule.followup` 연결을 통해 사용자 담당 고객 범위 안에서만 수집한다.
...
- 구현/로컬 검증 완료. 부서 AI는 `next_actions`, 개별 고객 AI는 `next_best_actions`에 단계별 fallback 액션을 보강한다.
