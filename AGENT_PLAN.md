...
## 2026-08-31 파이프라인: 수주 금액을 카드 위치가 아닌 실제 납품 기준으로

**Background**: 사용자 신고 — "이다민이 꾸준히 수주되고 있는데 견적제출로 이동되니까 수주가 아예 전부 없어짐. 실제 매출은 28,903,407원인데 파이프라인에서는 24,892,736원."

원인은 두 개였다.
1. `_select_pipeline_pricing` 이 **`stage == 'won'` 일 때만** 실제 납품액(`_actual_delivery_revenue`)을 읽는다. 카드를 견적으로 옮기는 순간 납품 기록을 아예 조회하지 않아 이미 납품된 매출이 통째로 사라진다. 프로덕션 확인: 이다민은 카드가 2개(68=견적, 521=수주)인데 납품 3건(2025-08-19 / 2026-02-25 558,415 / 2026-04-08 558,415)이 **전부 카드 68** 에 붙어 있고, 수주 칸의 521 에는 납품이 없다. 즉 실적을 가진 원본을 옮기고 빈 카드만 수주에 남은 상태.
2. 파이프라인과 대시보드가 **서로 다른 매출 정의**를 썼다. `_actual_delivery_revenue` 는 일정 없는 납품 활동까지 세지만 `delivered_revenue_rows` 는 세지 않는다 — 정의가 다르면 앞으로도 계속 갈라진다.

**Scope**:
- `reporting/funnel_views.py` `pipeline_command_center_api`: 수주 칸 금액을 대시보드 매출 카드와 **같은 헬퍼(`delivered_revenue_rows`) · 같은 사용자 범위(`_dashboard_scope_users`) · 같은 기간(올해)** 으로 통일. 카드 stage 와 무관.
  - 범위를 파이프라인 쪽 `_get_accessible_followups` 로 잡으면 안 된다 — 일정 소유자와 그 일정이 달린 FollowUp 소유자가 다른 건이 있어 값이 갈라진다(**실측: 일정 소유자 기준 28,903,407 vs FollowUp 소유자 기준 28,626,207**).
  - 연도 필터는 큐리셋에서 먼저 건다. `delivered_revenue_rows` 는 일정 1건마다 품목/활동을 훑으므로, 전 연도를 계산한 뒤 거르면 파이프라인 페이지에 헛일이 그대로 붙는다.
- `total_value` 를 `sum(stage_amounts.values())` 로 변경 — 수주 칸을 덮어썼으므로 카드 값을 그대로 더하면 화면의 칸 합계들과 총액이 어긋난다.
- 프론트(`App.tsx`): 수주 카드를 다른 단계로 끌면 확인창으로 "같은 담당자로 새 카드 만들기"를 안내(새 건이면 카드를 옮기는 게 아니라 하나 더 만드는 게 맞다). **선택된 카드는 잠재 칸의 "기본 접힘 + 상위 10건" 필터에 걸려도 항상 보이게** 한다 — 복사한 카드는 항상 잠재에서 시작하므로 곧바로 숨어 "복사가 안 된다"고 보였다(실제로는 생성돼 있었다).
- `RevenueDetailPage.tsx`: "취소되지 않은 선결제만 실제 매출로 집계" 각주 수정. 선결제를 매출에서 뺀 변경 이후 설명만 옛 규칙으로 남아 있었다.
- 테스트 2개 추가: 카드가 수주 칸을 떠나도 수주 금액이 유지되는지 / **파이프라인 수주 칸 == 대시보드 올해 매출**(두 화면이 다시 갈라지면 깨진다).

**DB change required**: No.

**검증**: 전체 회귀 **516개**, 실패 4건은 기존 무관 결함(익명유저 엑셀 익스포트 / SESSION_SAVE_EVERY_REQUEST / vat_mode 폼 / ai_chat 납품 활동 문구)뿐, 신규 회귀 없음. `tsc --noEmit` 클린, `npm run build` 성공. `scripts/post_deploy_smoke.py` → **ok (30/30 PASS)**. 대시보드 API **200** 확인.

**프로덕션 실측**: 파이프라인 수주 칸 **₩24,892,736 → ₩28,903,407**, 대시보드 올해 매출 ₩28,903,407 과 **일치**. 총 파이프라인 ₩38,214,577.

**Deploy**: Done. Commit `10e68d5` on `origin/main`. Railway `web` deploy `5293a938-34f0-49f7-b14d-adfff1771de1` SUCCESS.

---

## 2026-08-31 파이프라인 시트: 발표용으로 긴 노트 접기 + 프론트 배포 경로 복구

**Background**: 사용자 요청 — "파이프라인 시트는 발표용이야. 그런데 미팅 내용이랑 영업노트 내용이 그대로 붙어 있으니까 지저분해서 발표가 안 돼." 운영 데이터 기준 노트 본문 평균 **2,189자·최대 64줄**, 최근 40건 중 26건이 `[...]` 형태의 헤드라인으로 시작한다. 사용자가 고른 방식: **첫 줄만 보이고 클릭하면 펼침**, 장애물·다음 액션도 **같은 방식으로 정리**.

**Scope**:
- `PipelineSheetPage.tsx`: `CollapsibleActivityCell` 신설 — 상황/내용·장애물·다음 액션 3개 칸에 적용. 펼침 토글은 인라인 편집 버튼 **바깥**에 둔다(중첩 `button` 은 무효 HTML이라 편집 클릭이 깨진다).
- 임계값을 칸별로 나눈다(`body` 60자 / `obstacle`·`nextAction` 30자). 하나로 두면 폭이 좁은 장애물 칸에서 3줄로 접혀 행 높이가 다시 들쭉날쭉해진다(실측: 단일 임계값일 때 109px).
- `styles.css`: `.pipeline-sheet-clamp`(1줄 말줄임), `.pipeline-sheet-more-toggle`.
- **`railway.toml`: `watchPatterns` 에 `/frontend/**` 복구.** 2026-08-16 에 빌드 비용을 아끼려고 뺐지만, 뺄 당시엔 사용자 진입점이 별도 `sales-note-frontend` 서비스였고 **같은 날 그 서비스를 삭제**했다. 지금은 `web` 이 SPA 를 서빙하는 유일한 곳이라, 이 패턴이 없으면 프론트만 고친 커밋은 빌드가 아예 안 돌아 사용자에게 영원히 반영되지 않는다. 서비스가 하나뿐이라 "중복 빌드" 문제도 이제 없다.

**DB change required**: No.

**검증**: 프론트 전용 변경. `tsc --noEmit` 클린, `npm run build` 성공. 로컬 Django+Vite 브라우저 실측 — 접힘 상태에서 헤드라인 1줄 + "더보기", 펼침 시 전체 표시 및 clamp 해제, **행 높이 109px → 77px**, 모든 행 균일. 백엔드 회귀는 같은 배포의 위 항목에서 516개 확인.

**Deploy**: Done. Commit `f2b3232` on `origin/main`. Railway `web` deploy `5293a938-34f0-49f7-b14d-adfff1771de1` SUCCESS (두 커밋이 같은 배포에 포함).
...
카드는 2026-08-04 에 이미 건(FollowUp) 단위로 분리했지만, 실제로는 두 곳에서 막히고 있었다.

- **(a) 두 번째 카드를 만들 수 없음** — `followup_create_ajax` 가 (고객명+업체+부서+담당) 중복을 "이미 동일한 팔로우업이 존재합니다"로 거부했다. React 화면도 이 API(`/reporting/api/followups/create/`)를 쓴다. FollowUp 모델에는 유니크 제약이 없어 DB 차원 문제는 아니었다.
- **(b) 만들어도 끌려갈 위험** — 납품→수주 시그널(`_move_account_to_won`)에 "담당자 없이 부서로만 기록된 납품이면 그 부서 계정을 **전부** 수주로 이동"하는 분기가 있었다. 새로 만든 접촉/미팅 카드가 곧바로 수주로 끌려간다. 이동·숨김은 2026-08-04 에 이미 건 단위로 바꿨는데(`funnel_pipeline_move` / `_funnel_pipeline_set_hidden`) 이 시그널만 부서 단위로 남아 있었다.

**Scope**:
- `followup_create_ajax`: 중복 거부 제거. 파이프라인 카드는 "고객"이 아니라 "영업건" 단위이므로, 같은 사람과 동시에 여러 건을 진행하거나 한 건이 끝난 뒤 새 건을 시작하는 것은 정상 업무다. 사용자가 "제한 없이 허용"을 선택.
- `_move_account_to_won`: 부서 전체 이동 분기 제거. 부서만 걸린 납품은 그 부서의 **어느 건**인지 알 수 없으므로 아무 카드도 건드리지 않는다. 매출 집계는 단계와 무관하므로 카드를 안 옮겨도 그 납품은 매출로 잡힌다. 이제 안 쓰이는 `department_id` 인자도 제거.
- 테스트: 같은 담당자로 두 번째 카드 생성 / 수주 김지훈과 접촉 김지훈 공존 시 한쪽 납품이 다른 쪽을 안 끌고 감 / 부서 단위 납품이 다른 카드를 안 건드림. 부서 전체 이동을 검증하던 기존 테스트(`test_standalone_delivery_history_without_followup_advances_whole_department`)는 새 동작으로 교체.

**DB change required**: No.

**프로덕션 실측**: 김지훈 카드는 현재 **1개**(id 498, 건국대학교, `won`) — 두 번째 카드는 사용자가 UI 에서 직접 만든다. 담당자 없이 부서로만 기록된 납품은 **노트 1건 + 일정 1건**뿐이고 그 부서에 계정이 **0개**라, 이번 시그널 변경의 즉시 영향은 없다(잠재 위험만 제거).

**검증**: 전체 회귀 **509개** — 실패 4건은 기존 무관 결함(익명유저 엑셀 익스포트 / SESSION_SAVE_EVERY_REQUEST / vat_mode 폼 / ai_chat 부서분석 문구)뿐, 신규 회귀 없음. `post_deploy_smoke.py` → **ok (30/30 PASS)**. 대시보드 API **200** 확인(직전 세션에서 NameError 로 500 을 낸 적이 있어 배포마다 확인한다). 기존 김지훈 카드(498)가 `won` 그대로인 것도 확인.

**Deploy**: Done. Commit `80a14ca` on `origin/main`. Railway `web` deploy `dd65e01a-d5dd-4263-961c-ac23fda71ae4` SUCCESS.

---

## 2026-08-24 매출: 납품 1건 = 1회 계상 + 납품되면 자동 수주 전환

**Background**: 사용자 보고 — 대시보드 당해년도 매출(28,174,327)과 파이프라인 화면(24,604,360)이 다르다.

먼저 두 숫자는 **애초에 비교 대상이 아니었다**. 파이프라인의 24,604,360 은 "총 파이프라인"(보드 위 카드 값 합계)이라 아직 안 팔린 협상 건 2,747,635 가 섞여 있다. 다만 대조하는 과정에서 진짜 결함 4가지가 드러났다.

- **(a) 금액이 영업노트에만 있는 납품을 통째로 누락** — 일정 177: 일정 품목 0원인데 노트 품목 1,053,800원.
- **(b) 같은 납품을 일정·노트 양쪽에 적어둔 데이터 존재** — 일정 293: 양쪽 다 6,735,300원. 그래서 (a)를 고치겠다고 두 소스를 **더하면 이중계상**이 된다(사용자가 "일정이랑 영업노트랑 집계가 중복되면 안 된다"고 정확히 지적).
- **(c) 견적 일정에 품목을 넣고 납품노트를 달아 출고한 건 누락** — 일정 880/881, 올해 729,080원. `activity_type` 만 보면 안 잡힌다.
- **(d) 납품→수주 자동전환이 History 저장에만 걸려 있음** — 납품을 일정으로만 기록한 계정이 접촉/미팅에 잔류(5,652,652원어치: 인하대 지동환 3,255,312 / 건국대 김지훈 2,397,340). 자동전환 시그널은 2026-07-31 에 추가됐는데 두 계정의 납품노트는 6/26·7/21 로 그보다 앞섰다.

**Scope**:
- 신규 공용 헬퍼 `delivered_revenue_rows()` / `delivered_schedule_amount()` / `delivered_schedule_ids()`(views.py). **일정 1건당 한 번만** 세고, 금액은 `일정 품목 → 노트 품목 → 노트 delivery_amount → expected_revenue` 중 **먼저 잡히는 하나만** 쓴다. 두 소스를 절대 더하지 않는다(파이프라인 `_delivery_schedule_amount` 와 같은 규칙).
- 매출 대상 일정 = 완료 일정 중 `activity_type='delivery'` **또는 납품노트가 달린 것**.
- `dashboard_summary_api` 와 `revenue_detail_api` 가 **같은 헬퍼**를 거치게 함. 드릴다운 행도 품목 단위 → 일정 단위로 변경(품목 단위면 금액을 노트에서 읽어온 일정이 목록에 안 나와 합계가 어긋난다).
- 수주 자동전환: `Schedule` post_save 시그널 추가(완료된 납품 일정만, 예정은 제외). History/Schedule 공통 로직을 `_move_account_to_won()` 으로 묶음. 마이그레이션 **0125** 로 기존 잔여분 백필.
- `SchedulePipelineBackfillCommandTests` 픽스처 수정 — 이제 완료 납품 일정을 저장하면 시그널이 즉시 수주로 옮기므로, 레거시(시그널 이전) 상태는 시그널을 안 타는 `queryset.update()` 로 재현해야 커맨드를 테스트할 수 있다.

**DB change required**: Yes. `0125_backfill_won_for_delivered_schedules` — 스키마 변경 없음, `pipeline_stage` 백필 데이터 마이그레이션.

**사고 기록 (배포 직후 프로덕션 500)**: `f8307c8` 배포 직후 대시보드 API 가 `NameError: name 'Decimal' is not defined` 로 **500** 이 났다. `views.py` 는 `Decimal` 을 모듈 레벨이 아니라 함수 안에서 import 하는 파일인데 새 헬퍼가 그 관례를 안 따랐다. **테스트가 못 잡은 이유**: `item.total_price or Decimal('0')` 의 오른쪽은 금액이 0/None 일 때만 평가되는데 테스트 품목은 전부 금액이 있어 그 분기를 한 번도 안 탔다(프로덕션에는 금액이 빈 품목이 있었다). 핫픽스 `7bdf028` 로 import 추가 + 금액이 빈 품목 회귀 테스트 추가. **교훈: or-fallback 은 fallback 이 실제로 평가되는 데이터로 테스트해야 한다.**

**검증**: 전체 회귀 506개 — 실패 4건은 기존 무관 결함(익명유저 엑셀 익스포트 / SESSION_SAVE_EVERY_REQUEST / vat_mode 폼 / ai_chat 부서분석 문구)뿐, 신규 회귀 없음. 신규 테스트 6개(일정·노트 중복 시 1회만 / 노트에만 있는 금액 / 견적일정 출고 / 금액 빈 품목 / 일정만으로 납품 시 수주 전환 / 예정 상태에서는 전환 안 함). 배포 로그에 `Applying reporting.0125_backfill_won_for_delivered_schedules... OK` 확인. `post_deploy_smoke.py` → **ok (30/30 PASS)**.

**프로덕션 실측 (배포 전 → 후)**:

| 항목 | 전 | 후 |
|---|---|---|
| 대시보드 매출(dkswogus95, 올해) | 28,174,327 | **28,903,407** |
| 파이프라인 수주 | 21,856,725 (21건) | **27,509,377 (23건)** |
| FollowUp 493 인하대 지동환 | contact | **won** |
| FollowUp 498 건국대 김지훈 | contact | **won** |

**Deploy**: Done. Commit `f8307c8` + 핫픽스 `7bdf028` on `origin/main`. Railway `web` deploy `1c138fce-f7d5-473e-afe6-b495ec0c6d92`(마이그레이션 적용), `eead73b3-31ef-4acb-a3bc-80d89a58d741`(핫픽스) 둘 다 SUCCESS. 대시보드 API 200 복구 확인.
...
- ~~**`salesnote-mcp` 슬립 활성화 (−약 $0.86/월)**~~ — **2026-08-16 당일 되돌림. 다시 켜지 말 것.**
  켠 직후 사용자가 "CRM 커넥터가 연결이 안 된다"고 보고했다. 원인: 슬립 중인 서비스의 첫 요청은
  502 가 나는데(Railway 문서 명시), claude.ai 커스텀 커넥터는 그걸 재시도가 아니라 **연결 실패로
  판정해 커넥터를 끊어버린다**. 이 세션에서도 커넥터 도구(`salesnote_read/search/write`)가 실제로
  사라졌다. 엔드포인트 자체는 멀쩡했고(수동 요청으로 깨우면 200 정상 핸드셰이크) 오직 슬립이 원인.
  월 $0.86 아끼려고 사용자가 매일 쓰는 CRM 커넥터를 끊는 건 명백한 손해라 `sleep_application=false`
  로 복구했다. **교훈: 외부 서비스(claude.ai 커넥터 등)가 붙는 엔드포인트에는 슬립을 걸면 안 된다.**
  Railway 슬립은 "첫 요청 502 를 클라이언트가 재시도해 주는" 경우에만 쓸 수 있다.
...
**결과**: 월 **$5.06 → 약 $4.2** (프론트 서비스 제거분 $1.00 만 유효). 서비스 4개 → 3개.
MCP 슬립분($0.86)은 커넥터가 끊겨 당일 되돌렸다(위 참조).
