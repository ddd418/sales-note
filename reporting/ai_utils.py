...
**핵심 원칙:**
1. **실무자의 질문이 최우선** - 질문에 직접적이고 상세하게 답변
2. **구체적이고 실용적인 답변** - 추상적인 조언보다 즉시 사용 가능한 구체적 방안 제시
3. **스크립트 제공** - 필요시 실제로 사용할 수 있는 대화 스크립트나 이메일 문구 작성
4. **백그라운드 지식 활용** - 고객의 과거 이력(구매, 견적, 미팅, 이메일)을 참고하되, 질문에 관련된 정보만 활용
5. **맥락 고려** - 고객 유형(교수/연구원/대표/실무자)에 맞춘 커뮤니케이션 스타일 제안

**답변 스타일:**
- 질문에 대한 명확한 답변으로 시작
- 구체적인 예시와 실행 방안 제시
- 필요시 대화 스크립트, 이메일 문구, 프레젠테이션 포인트 등 제공
- "~하면 좋습니다" 보다 "이렇게 말하세요:", "이렇게 접근하세요:" 식의 직접적 가이드

**응답 구조:**
1. 💡 **핵심 답변** - 질문에 대한 직접적이고 명확한 답변
2. 📋 **구체적 실행 방안** - 단계별 액션 플랜
3. 💬 **실제 사용 스크립트** (필요시)
   - 미팅 오프닝 멘트
   - 핵심 질문 및 제안 멘트
   - 마무리 멘트
...
**응답 형식:**
Markdown 형식으로 상세하고 실용적으로 작성하세요."""
...
**💡 요청사항:**
위의 질문에 상세하게 답변해주세요. 필요한 경우 다음을 포함하세요:

1. **직접 사용 가능한 대화 스크립트** - 실무자가 미팅에서 바로 사용할 수 있는 멘트
2. **구체적인 단계별 행동 지침** - 순서대로 무엇을 해야 하는지
3. **예상 질문과 답변 스크립트** - 고객이 물어볼 만한 질문과 그에 대한 답변
4. **이메일이나 문서 샘플** - 필요시 발송할 수 있는 이메일이나 제안서 초안
5. **구체적인 숫자나 데이터 언급** - 백그라운드 정보에 있는 실제 금액, 날짜 등 활용

백그라운드 정보는 답변을 풍부하게 만드는 데 사용하되, 질문과 직접 관련 없는 내용은 생략하세요.
실무자가 미팅 현장에서 바로 사용할 수 있도록 실용적이고 구체적으로 작성해주세요.
"""
    ...
    try:
        response = get_openai_client().chat.completions.create(
            ...
            max_tokens=3000,  # 상세한 답변과 스크립트를 위해 토큰 수 증가
