...
urlpatterns = [
    ...
    re_path(
        r'^(?P<path>(?:data-cleanup|downloads|services|assets)(?:/.*)?|accounts/\d+/cleanup-preview(?:/.*)?)$',
        frontend_views.removed_react_route,
        name='removed_react_route',
    ),
    re_path(
        r'^(?P<path>(?:dashboard|customers|accounts|demos|notes|schedules|employees|mailbox|business-cards|weekly-reports|documents|products|receivables|prepayments|profile|pipeline|companies)(?:/.*)?)$',
