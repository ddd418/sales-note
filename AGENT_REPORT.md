...
→ Pushed commit b9e3a45 to main

railway deployment list --service sales-note-frontend --limit 1 --json
→ sales-note-frontend deployment 0f25bbd6-1571-48f5-87bb-beed256eafe1 SUCCESS

railway deployment list --service web --limit 1 --json
→ web deployment 6a39b04f-015b-4407-8bfc-d97c6483cea1 SUCCESS
...
- Runtime commit `b9e3a45` 배포 완료.
- `sales-note-frontend`: `0f25bbd6-1571-48f5-87bb-beed256eafe1` SUCCESS.
- `web`: `6a39b04f-015b-4407-8bfc-d97c6483cea1` SUCCESS.
- 운영 smoke test 통과.
...
git push origin main
→ Pushed commit 50bcb87 to main

railway deployment list --service sales-note-frontend --limit 1 --json
→ sales-note-frontend deployment 06efb61a-6004-4374-8c7b-f2777cd80fc8 SUCCESS

railway deployment list --service web --limit 1 --json
→ web deployment eda6a735-897f-4764-b1be-c843ea50fe51 SUCCESS
...
- Runtime commit `50bcb87` 배포 완료.
- `sales-note-frontend`: `06efb61a-6004-4374-8c7b-f2777cd80fc8` SUCCESS.
- `web`: `eda6a735-897f-4764-b1be-c843ea50fe51` SUCCESS.
- 운영 smoke test 통과.
...
git push origin main
→ Pushed commit 8333f5b to main

railway deployment list --service sales-note-frontend --limit 1 --json
→ sales-note-frontend deployment 740ebed4-2e79-4d05-8c12-ef1d73cb6c44 SUCCESS

railway deployment list --service web --limit 1 --json
→ web deployment 469885a2-0126-4ce3-8c10-eb15a73d18ff SUCCESS
...
- Runtime commit `8333f5b` 배포 완료.
- `sales-note-frontend`: `740ebed4-2e79-4d05-8c12-ef1d73cb6c44` SUCCESS.
- `web`: `469885a2-0126-4ce3-8c10-eb15a73d18ff` SUCCESS.
- 운영 smoke test 통과.
...
- `b5738a4 fix: search account cleanup targets` push 완료.
- Railway web deployment `b5a73665-24a7-4b59-bb91-d5cfc23ce401` SUCCESS.
- Railway frontend deployment `dcd3c9d9-eb98-437f-b867-943a12733c75` SUCCESS.
- 운영 frontend route `/accounts/1/cleanup-preview/` HTTP 200 확인.
- 운영 frontend bundle `/assets/index-Bau9ISag.js`와 `/assets/index-CXGWPmXK.css` 반영 확인.
- 운영 `/reporting/api/accounts/search/?q=test` 미인증 요청은 401로 보호됨 확인.
...
- React 고객 상세 `/customers/<id>/`에 `고객 운영 기록` 섹션을 추가했습니다.
- 서비스 기록, 견적 기록, 납품 기록, 선결제 기록을 한 화면에서 볼 수 있게 했습니다.
- 납품 기록은 `Schedule.use_prepayment`, `Schedule.prepayment`, `Schedule.prepayment_amount`, `PrepaymentUsage` 구조화 데이터로만 `선결제 차감 납품`과 `일반 납품`을 구분합니다.
- 고객 상세 API/React 화면에는 고객 AI 분석 payload/패널을 추가하지 않았고, 기존 테스트로 `aiDepartment` 미포함을 유지했습니다.
...
- 고객 상세에서 해당 고객의 납품 품목, 금액, 상태, 선결제 차감 여부를 바로 확인할 수 있습니다.
- 견적 기록과 서비스/A/S 기록도 같은 화면에 노출되어 고객 히스토리 확인 동선이 짧아졌습니다.
- 선결제 기록은 입금일, 입금자, 총액, 잔액, 사용액, 사용내역 수를 함께 보여줍니다.

### 기존 기능 보존

- DB 모델과 migration 변경은 없습니다.
- 기존 `reporting` 앱과 `/reporting/*` 라우트는 유지했습니다.
- 기존 고객 기본정보/주소/상세내용, 장비/서비스/교정 관리, 선결제 요약, 노트/일정 링크는 유지했습니다.
- AI Workspace와 파이프라인의 AI 기능은 그대로 두고, 고객 상세에는 AI 패널을 두지 않았습니다.
...
python manage.py test reporting.tests.CustomersSummaryApiTests.test_customer_detail_summary_api_includes_operational_records_with_payment_source reporting.tests.CustomersSummaryApiTests.test_customer_detail_summary_api_excludes_customer_ai_payload --verbosity=1
→ Ran 2 tests, OK
...
cd frontend; npm run build
→ OK, dist/assets/index-v1Q3oWGy.js / dist/assets/index-Dt0JE5if.css generated
→ Vite chunk-size warning only
...
git commit -m "feat: show customer operational records"
→ 4ef8b66
...
Railway deployment list --service web
→ 90c4b478-be5a-4aa5-82bb-ce5c1887ba89 SUCCESS, commit 4ef8b66

Railway deployment list --service sales-note-frontend
→ b0eb3f40-fdd2-4557-9108-4c03d5daaa83 SUCCESS, commit 4ef8b66

Production smoke
→ /customers/156/ 200
→ /reporting/api/customers/156/ anonymous GET 401 login_required
→ latest JS bundle contains "고객 운영 기록", "납품 기록", "선결제 차감 여부 표시", "일반 납품"
```

### 알려진 제한

- CRM에는 아직 별도 `일반결제` 확정 필드가 없습니다. 화면의 `일반 납품`은 구조화된 선결제 차감 기록이 없는 납품이라는 뜻입니다.
- 운영 데이터 내용 확인은 로그인 세션이 필요하므로 자동 production smoke는 라우트/번들/API 보호까지만 확인했습니다.
- 고객 상세에는 AI 패널을 두지 않았지만, 전역 사이드바의 AI Workspace 메뉴와 파이프라인 AI 기능은 기존 기능으로 남아 있습니다.

### 권장 다음 작업

- 사용자가 `/customers/156/`에서 실제 운영 데이터를 확인한 뒤, 필요하면 납품 기록 필터 또는 부서 단위 기록 포함 여부를 추가 조정합니다.

### 운영 배포 상태

- 런타임 커밋 `4ef8b66 feat: show customer operational records` 푸시 완료.
- Railway `web` 배포 `90c4b478-be5a-4aa5-82bb-ce5c1887ba89` 성공.
- Railway `sales-note-frontend` 배포 `b0eb3f40-fdd2-4557-9108-4c03d5daaa83` 성공.
- 운영 URL: `https://sales-note-frontend-production.up.railway.app`

### 수동 서버 테스트 절차

1. `https://sales-note-frontend-production.up.railway.app/customers/156/`에 로그인 상태로 접속합니다.
2. 고객 기본정보 아래 `고객 운영 기록` 섹션이 보이는지 확인합니다.
3. `납품 기록`에서 각 납품 행에 `선결제 차감 납품` 또는 `일반 납품` 배지가 보이는지 확인합니다.
4. 선결제 차감 납품은 차감 금액과 품목/일정 링크가 실제 선결제 사용 내역과 맞는지 확인합니다.
5. `서비스 기록`, `견적 기록`, `선결제 기록` 목록이 해당 고객 기준으로 보이는지 확인합니다.
6. 고객 상세 화면 안에 AI 분석 카드/AI 질문 패널이 없는지 확인합니다.
...
AI가 이미 답장이 온 김명환 연구원 메일을 `답장 유무 확인` 대상으로 다시 추천하는 문제를 수정했습니다. 기존에는 받은 메일이 같은 Gmail/thread/message ID로 들어온 경우만 답장으로 인식했기 때문에, 실제 답장이 CRM에 있어도 스레드 ID가 다르게 저장되면 `email_waiting` 액션이 남을 수 있었습니다.

이제 보낸 메일 이후 받은 메일을 판단할 때 기존 thread/message ID 매칭을 유지하면서, 같은 고객, 같은 정규화 제목, 보낸 메일 수신자와 받은 메일 발신자가 맞는 경우도 답장으로 인정합니다.

### 변경된 파일

- `reporting/views.py`: `email_waiting` 액션 생성 전 받은 답장 존재 여부를 더 넓게 검사하는 helper 추가.
- `reporting/tests.py`: 김명환 스타일의 “다른 thread ID로 저장된 답장” 회귀 테스트 추가.
- `AGENT_PLAN.md`, `AGENT_REPORT.md`: 계획과 결과 기록.

### CRM 개선

- 이미 답장이 온 고객을 `메일 답장 확인` 대상으로 다시 올리는 오탐을 줄였습니다.
- Gmail thread ID가 달라져도 같은 고객/제목/참여자 기준으로 답장을 인식합니다.
- 답장이 없는 기존 보낸 메일은 계속 `email_waiting` 액션으로 남습니다.

### 기존 기능 보존

- 기존 thread/message ID 기반 답장 인식은 그대로 유지했습니다.
- 같은 제목/수신자 기준 dedupe 로직도 유지했습니다.
- DB schema 변경은 없습니다.
...
python manage.py test reporting.tests.AIWorkspaceSummaryApiTests.test_ai_workspace_action_queue_includes_sent_email_without_received_reply reporting.tests.AIWorkspaceSummaryApiTests.test_ai_workspace_action_queue_skips_email_waiting_when_reply_has_different_thread_id reporting.tests.AIWorkspaceSummaryApiTests.test_ai_workspace_action_queue_dedupes_email_waiting_by_thread_or_subject --verbosity=2
→ Ran 3 tests, OK
...
- 받은 메일이 고객/팔로우업에 전혀 연결되지 않은 상태라면 아직 답장으로 판별할 수 없습니다. 메일 연결 자체가 빠진 경우는 별도의 메일-고객 자동 연결 개선이 필요합니다.
- 제목이 완전히 바뀐 새 메일은 답장이 아니라 별도 메일로 판단합니다.

### 권장 다음 단계

운영 AI 메뉴에서 같은 질문을 다시 실행해 김명환 연구원이 `메일 답장 확인` 항목으로 남지 않는지 확인합니다. 계속 남는다면 해당 받은 메일이 고객/팔로우업에 연결되어 있는지 확인하는 자동 연결 보강이 다음 단계입니다.

### 운영 수동 검수 절차

1. 운영 프론트에 로그인합니다.
2. AI 메뉴에서 전체 부서 범위로 `오늘 내가 해야할 일이 있을까?? 이다민 연구원이랑 김기윤연구원 한은영교수 제외하고`를 다시 질문합니다.
3. 김명환 연구원이 `답장 유무 확인`, `메일 답장 확인`, `리마인드 메일` 같은 항목으로 추천되지 않는지 확인합니다.
4. 메일함에서 김명환 받은 메일이 해당 고객/팔로우업과 연결되어 있는지도 함께 확인합니다.
...
- 2026-05-18 사용자가 `검수완료`로 운영 수동검수 완료를 확인했습니다.
...
컴퓨터 장애 가능성에 대비해 다음 작업자가 바로 이어받을 수 있도록 `HANDOFF.md`를 최신 상태로 갱신했습니다.

### 변경된 파일

- `HANDOFF.md`: 최신 완료 작업, 배포 상태, 운영 스모크, 수동 검수 절차, 남은 요청, 주요 코드 위치, 검증/배포 명령 정리.
- `AGENT_REPORT.md`: 인수인계 문서 갱신 기록 추가.

### 기존 기능 보존

- 문서 변경만 수행했습니다.
- DB 모델 변경, migration, runtime 배포는 필요 없습니다.

### 알려진 제한

- 최신 대시보드/파이프라인 운영 수동 검수는 아직 사용자 확인이 필요합니다.

### 권장 다음 작업

- 사용자가 최신 운영 검수를 확인하면 AI Workspace 전체 CRM 질문 또는 PI 이름 기반 부서 검색 작업을 이어갑니다.
...
`/schedules/903/` 납품 품목 편집에서 선결제 차감을 켰을 때, 아직 견적/납품 품목 합계가 0원인데도 선결제 row가 선택 가능해서 `품목 상한 ₩0 · 최대 차감 ₩0`으로 보이는 문제를 수정했습니다.

이제 차감할 품목 합계가 0원이면 선결제 선택/차감 입력/`전체 차감`을 비활성화하고, 같은 영역에 `견적 품목 불러오기` 버튼을 보여줍니다. 견적 품목을 가져오거나 품목 금액을 입력하면 기존처럼 품목 상한과 최대 차감액이 계산됩니다.

### 변경된 파일

- `frontend/src/App.tsx`: 품목 합계 0원 상태의 선결제 차감 차단, 견적 품목 불러오기 CTA, 저장 전 검증 추가.
- `frontend/src/styles.css`: 선결제 품목 필요 안내 안의 버튼 스타일 보정.
- `AGENT_PLAN.md`, `AGENT_REPORT.md`: 작업 계획과 결과 기록.

### CRM 개선

- 품목이 없는 상태를 차감 가능 상태처럼 보여주지 않습니다.
- 선결제 차감 흐름에서 바로 견적 품목 불러오기로 이동할 수 있습니다.
- 납품 품목 합계가 있어야만 선결제 차감액을 입력/저장할 수 있습니다.

### 기존 기능 보존

- DB 모델 변경과 migration은 없습니다.
- `/reporting/*` backend route는 변경하지 않았습니다.
- 기존 견적 품목 불러오기, 다중 견적 선택, 선결제 차감 API payload 구조는 유지했습니다.
...
cd frontend; npm run build
→ OK, assets/index-BTb0sU0h.js / assets/index-GdZLLCy-.css, Vite chunk-size warning only
...
- 운영 실제 데이터에서 견적 품목 목록 표시와 적용은 로그인 세션이 필요해 사용자 수동 검수가 필요합니다.
...
1. 운영에서 `/schedules/903/`에 로그인 상태로 접속합니다.
2. 납품 품목 편집을 열고 `납품 저장 시 선결제 차감`을 켭니다.
3. 아직 품목 합계가 0원이면 `차감할 납품 품목 합계가 없습니다` 안내와 `견적 품목 불러오기` 버튼이 보이는지 확인합니다.
4. 그 버튼으로 견적 품목을 불러오고, 견적을 선택해 적용합니다.
5. 선결제를 선택하면 `품목 상한`, `최대 차감`, `입력 후 남은 납품금액`이 0원이 아닌 납품 합계 기준으로 표시되는지 확인합니다.
...
- DB 모델 변경과 migration은 없습니다.
- `/reporting/*` 기존 route와 레거시 화면은 유지했습니다.
- 견적 일정의 납품품목 편집에는 선결제 차감 UI를 노출하지 않았습니다.
- 완료/숨김 처리된 AI 액션은 기존 feedback 숨김 정책으로 계속 차단합니다.
...
- 사용자가 추가로 요청한 `/ai-workspace/` 전체 CRM 질문형 AI와 `/mailbox/` 엑셀 붙여넣기 기반 대량 메일 자동화는 이번 긴급 선결제/`action_not_found` 수정 범위에 포함하지 않았습니다. 이 작업 완료와 운영 검수 후 다음 작업으로 진행합니다.
- 운영 수동 검수는 실제 계정/실제 선결제 데이터가 필요합니다.
...
- 사용자가 추가 요청한 `/ai-workspace/` 전체 CRM 질문형 AI를 먼저 설계/구현하고, 이후 `/mailbox/` 엑셀 붙여넣기 기반 대량 메일 자동화로 진행합니다.
...
- 완료: 사용자가 2026-05-13 KST에 운영 검수를 완료했다고 확인했습니다.

### 권장 다음 작업

- 실제 템플릿 샘플 중 여전히 줄이 부족한 특정 셀이 발견되면 해당 템플릿 위치를 기준으로 추가 보정합니다.
...
- 2026-05-13 사용자가 운영 검수 완료를 확인했습니다.

### 권장 다음 작업

- React 일정 상세의 견적/납품 노트 생성 경계 문구를 더 명확히 하는 작은 UX 보강을 검토합니다.
...
운영 프론트에서 새 테스트 고객과 테스트 일정을 생성해 일정 메일 자동첨부 및 React 일정 캘린더 등록/수정/삭제 흐름을 확인했습니다. 실제 메일 발송은 하지 않고 발송 직전 자동첨부 표시/제거 상태까지만 검수했습니다.

### 사용한 테스트 데이터

- 테스트 고객: `466` (`CODEx 검수고객 20260512230046`)
- 견적 자동첨부 검수 일정: `884`
- 거래명세서 자동첨부 검수 일정: `885`
- 캘린더 등록/수정/삭제 검수 일정: `886`

### 검수 결과

- 납품 일정 `885` 메일 작성 화면에서 `거래명세서 PDF 자동 생성` 자동첨부 후보가 표시됐습니다.
- 거래명세서 자동첨부 제거 후 `자동 첨부를 모두 제외했습니다.` 상태가 표시됐습니다.
- 견적 일정 `884` 메일 작성 화면에서 `견적서 PDF 자동 생성` 자동첨부 후보가 표시됐습니다.
- 견적서 자동첨부 제거 후 후보 텍스트가 사라지고 제외 상태가 표시됐습니다.
- `/schedules/calendar/`에서 고객 일정 `886`을 선택 날짜 `2026-05-13` 기준으로 등록했습니다.
- 등록 후 캘린더 카드와 월간 일정 수가 갱신됐습니다.
- 일정 `886` 수정 패널에서 장소/메모를 변경했고 카드에 `CODEx 캘린더 수정 검수 완료`가 반영됐습니다.
- 일정 `886` 삭제 확인 모달을 승인한 뒤 카드가 사라지고 월간 일정 수가 되돌아갔습니다.

### 정리 결과

- 일정 `884`, `885`는 AJAX 삭제 API로 삭제 완료했습니다.
- 일정 `886`은 React 캘린더 UI 삭제로 삭제 완료했습니다.
- 테스트 고객 `466`은 삭제 완료했고, `/reporting/api/customers/466/`가 `404`를 반환하는 것을 확인했습니다.
- 캘린더 API에서 `884`, `885`, `886` 및 `CODEx` 테스트 일정이 남아 있지 않음을 확인했습니다.

### 실행한 확인

```text
Playwright 운영 브라우저
→ 로그인 세션으로 /mailbox/?compose=1&schedule_id=885 확인
→ 거래명세서 자동첨부 표시/제거 확인
→ /mailbox/?compose=1&schedule_id=884 확인
→ 견적서 자동첨부 표시/제거 확인
→ /schedules/calendar/에서 고객 일정 등록/수정/삭제 확인
...
- React 개인 일정 등록/수정 API를 추가해 캘린더에서 개인 일정 fallback을 줄입니다.

---

## 2026-05-13 — React Schedule Calendar Inline Actions

**상태**: 구현/로컬 검증/커밋/푸시/운영 배포/스모크 완료, 운영 수동검수 완료
...
| `AGENT_REPORT.md` | 배포 결과와 운영 수동검수 절차 기록 |
...
git commit -m "feat: add schedule commercial checks"
→ `815a6a0`

git push origin main
→ OK, `main` updated to `815a6a0`

railway redeploy --service web --from-source --yes --json
→ OK

railway up .\frontend --path-as-root --service sales-note-frontend --detach --json --message "Deploy commercial checks frontend 815a6a0"
→ OK, frontend deployment `5930b24d-a28a-40ed-82a4-7bcfa0211ee3`
```
...
- Production API auth smoke:
  ...
  - `GET https://sales-note-frontend-production.up.railway.app/reporting/api/schedules/1/` returned 401 through the frontend proxy for anonymous access.
- Railway service status: `web`, `sales-note-frontend`, and `Postgres` Online.
- Railway logs checked: backend gunicorn started, frontend process started, no 500 HTTP logs found during smoke; anonymous schedule API 401 entries were expected.
...
## AI Workspace 2.0 Action Center — AI 영업 지휘석 1단계 (2026-05-13)

### 1. Summary

- 운영 `/ai-workspace/` 조사와 AI 시장 트렌드 비교 계획을 바탕으로 React AI Workspace를 액션센터형 화면으로 확장했습니다.
- 기존 AI 분석/프롬프트 허브 위에 `Daily sales brief`, `AI 추천 실행 목록`, `Draft copilot`을 추가했습니다.
- 백엔드는 기존 CRM 데이터만 읽어 견적 후속, 납품 리스크, 고객 후속, PainPoint 검증, 주간보고 누락 액션을 계산합니다.
- 새 초안 API는 메일/노트/질문/주간보고 초안을 반환하지만, 사용자가 승인하기 전에는 CRM 데이터 저장이나 메일 발송을 하지 않습니다.

### 2. Files Changed

| 파일 | 변경 내용 |
| ---- | --------- |
| `AGENT_PLAN.md` | AI Workspace 2.0 1단계 범위와 no-migration 판단 기록 |
| `reporting/views.py` | action queue/daily brief 계산, 초안 생성 fallback/OpenAI 연동, draft POST API 추가 |
| `reporting/urls.py` | `/reporting/api/ai-workspace/actions/draft/` 추가 |
| `reporting/tests.py` | action queue, 권한, fallback 초안 API 회귀 테스트 추가 |
| `frontend/src/api.ts` | action queue/daily brief/draft 타입과 draft API client 추가 |
| `frontend/src/App.tsx` | AI Workspace 상단 브리프, 액션 카드, 초안 미리보기/복사 UI 추가 |
| `frontend/src/styles.css` | AI 브리프/액션/초안 패널 스타일 추가 |

### 3. CRM Improvements

- 영업사원이 `/ai-workspace/`에서 오늘 먼저 처리할 영업 액션을 우선순위 점수와 함께 확인할 수 있습니다.
- 견적 금액, 유효기한, 납품 예정일, 다음 액션, PainPoint 검증 질문 등 근거가 액션 카드에 함께 표시됩니다.
- 액션 카드에서 바로 메일/영업노트/질문/주간보고 초안을 생성하고 복사할 수 있습니다.
- OpenAI 키 또는 호출 실패 시에도 근거 기반 기본 초안을 반환해 운영 흐름이 완전히 막히지 않습니다.
- 기존 부서 분석 실행, 고객 분석 대상, PainPoint 검증, 추천 프롬프트, 추천 목표 흐름은 유지했습니다.

### 4. Existing Functionality Preserved

- 기존 `/reporting/*`, `/ai/*`, React `/ai-workspace/` 라우팅을 유지했습니다.
- 비로그인 API 접근은 401 `login_required`로 보호됩니다.
- `can_use_ai=False` 사용자는 기존처럼 AI 데이터와 초안 API를 사용할 수 없습니다.
- 1단계는 DB 필드와 migration을 추가하지 않았습니다.
- 초안 생성은 읽기/생성 응답만 수행하며 CRM 저장, 메일 발송, 일정 생성 같은 side effect는 없습니다.
...
git log --oneline -3
→ d950daa docs: record quote import guard deployment
→ 2368e8e feat: guard quote import delivery quantities
→ bf669ab docs: clarify commercial checks handoff
...
cd frontend && npm run build
→ OK, dist/assets/index-BRW3u8CT.js / dist/assets/index-CBFJriTJ.css generated
→ Vite chunk size warning only
...
git commit -m "feat: add AI workspace action center"
→ b516e11

git push origin main
→ main updated to b516e11

railway redeploy --service web --from-source --yes --json
→ OK

railway up .\frontend --path-as-root --service sales-note-frontend --detach --json --message "Deploy AI workspace action center b516e11"
→ OK, frontend deployment 8a36e5eb-ee5f-4038-a0ed-1d98fb848875
```

### 6. Local Smoke

- `http://127.0.0.1:5173/ai-workspace/`를 Playwright CLI로 열었습니다.
- 비로그인 상태에서 `/reporting/api/ai-workspace/`가 401을 반환하고, 화면이 `/reporting/login/?next=%2Fai-workspace%2F`로 이동하는 것을 확인했습니다.
- 브라우저 콘솔에는 로컬 `favicon.ico` 404와 비로그인 API 401만 관찰됐습니다.

### 7. Known Limitations

- 운영 로그인 세션이 없어 실제 운영 고객 데이터가 들어간 Action Queue 화면은 자동 검수하지 못했습니다.
- 1단계는 계산형 action queue입니다. AI 추천 클릭/저장/무시/완료 로그와 ROI 측정용 `AIActionLog`는 아직 없습니다.
- 초안 API는 승인 전 저장하지 않으므로 실제 영업노트/메일/보고서 저장은 사용자가 기존 화면에서 수행해야 합니다.

### 8. Recommended Next Task

M4: `AIActionLog`와 저장/무시/완료 피드백 루프. AI 액션 생성, 초안 복사, CRM 저장, 무시, 후속 완료를 기록해 추천 품질과 매출 기여도를 측정하는 단계가 적절합니다.

### 9. Production Deployment Status

- Implementation/report commit: `b516e11 feat: add AI workspace action center`
- GitHub push: `main` updated to `b516e11`
- Railway `web`: `f950459d-638c-4a9e-aae8-e5157b25e049` SUCCESS, commit `b516e11`
- Railway `sales-note-frontend`: `8a36e5eb-ee5f-4038-a0ed-1d98fb848875` SUCCESS
- Production frontend smoke:
  - `GET https://sales-note-frontend-production.up.railway.app/ai-workspace/` returned 200.
  - Served `/assets/index-BRW3u8CT.js` and `/assets/index-CBFJriTJ.css`.
  - JS bundle contains `AI 추천 실행 목록` and `/reporting/api/ai-workspace/actions/draft/`.
  - CSS bundle contains `ai-action-card`.
- Production API auth smoke:
  - `GET https://sales-note-frontend-production.up.railway.app/reporting/api/ai-workspace/` returned 401.
  - `GET https://web-production-5096.up.railway.app/reporting/api/ai-workspace/` returned 401.
- Railway logs checked:
  - `web` migrations completed with no migrations to apply; gunicorn started.
  - `sales-note-frontend` container started.
...
2. 로그인 후 상단 `오늘의 AI 영업 지휘석`에 추천 액션/긴급/견적 후속/고객 후속 지표가 표시되는지 확인합니다.
3. `AI 추천 실행 목록`에서 견적 후속, 납품 리스크, 고객 후속, PainPoint 검증, 주간보고 액션이 실제 CRM 데이터와 맞는지 샘플 5건을 확인합니다.
4. 액션 카드에서 `메일`, `노트`, `질문`, `보고` 초안을 생성하고 초안이 자동 저장/발송되지 않는지 확인합니다.
5. `초안 복사` 후 기존 영업노트/메일/주간보고 흐름에 붙여 넣어 사용할 수 있는지 확인합니다.
6. 기존 `AI 분석 실행`, `PainPoint 검증 메모`, `AI 허브`, `주간보고`, `고객`, `영업노트` 링크가 계속 동작하는지 확인합니다.

---

## AI Workspace Sold Quote Exclusion Hotfix — 완료/판매 견적 지휘석 제외 (2026-05-13)

### 1. Summary

- 사용자 제보: 운영 `https://sales-note-frontend-production.up.railway.app/schedules/415/`는 이미 완료된 견적/판매 건인데 AI 지휘석의 견적 후속 액션에 표시됐습니다.
- 원인: AI action queue가 `Quote.converted_to_delivery=False`와 fallback 견적 일정만 보고, 연결된 견적 일정의 `completed` 상태를 충분히 제외하지 않았습니다.
- 조치: 완료된 견적 일정은 Quote flag가 남아 있어도 action queue와 열린 견적 프롬프트 금액 계산에서 제외했습니다.

### 2. Files Changed

| 파일 | 변경 내용 |
| ---- | --------- |
| `AGENT_PLAN.md` | 완료/판매 견적 제외 hotfix 범위 기록 |
| `reporting/views.py` | 완료 견적 일정 exclusion 필터 추가 |
| `reporting/tests.py` | 완료/판매 견적이 action queue와 프롬프트 열린 견적 문맥에 나오지 않는 회귀 테스트 추가 |

### 3. CRM Improvements

- 이미 판매 완료된 견적이 `AI 추천 실행 목록`의 `견적 후속`으로 다시 뜨는 오탐을 줄였습니다.
- Quote 객체의 전환 flag가 미정리된 데이터라도 일정 상태가 `completed`이면 판매 완료로 보고 AI 후속 대상에서 제외합니다.

### 4. Existing Functionality Preserved

- 기존 AI Workspace, draft API, PainPoint 검증, `/reporting/*` 인증/권한 정책은 유지했습니다.
- 아직 진행 중인 `scheduled` 견적 일정은 계속 AI 후속 후보로 남습니다.
- DB 필드와 migration은 추가하지 않았습니다.
- 프론트엔드 변경은 없습니다.
...
git commit -m "fix: exclude sold quotes from AI action queue"
→ aa314b6

git push origin main
→ main updated to aa314b6
...
- Hotfix commit: `aa314b6 fix: exclude sold quotes from AI action queue`
- GitHub push: `main` updated to `aa314b6`
- Railway `web`: `7f4f776d-3f9a-4020-b9a8-867c23217afe` SUCCESS, commit `aa314b6`
- Railway `sales-note-frontend`: 변경 없음
- Production API auth smoke:
  - `GET https://sales-note-frontend-production.up.railway.app/reporting/api/ai-workspace/` returned 401.
  - `GET https://web-production-5096.up.railway.app/reporting/api/ai-workspace/` returned 401.
- Railway logs checked: migrations completed with no migrations to apply; gunicorn started.

### 7. Manual Server Test Process

1. 운영 프론트에서 `https://sales-note-frontend-production.up.railway.app/schedules/415/`가 완료/판매 상태인지 다시 확인합니다.
2. `https://sales-note-frontend-production.up.railway.app/ai-workspace/`로 이동합니다.
3. `AI 추천 실행 목록`에서 schedule `415` 또는 해당 고객의 완료 견적이 `견적 후속`으로 뜨지 않는지 확인합니다.
4. 아직 `scheduled` 상태인 미완료 견적은 계속 `견적 후속` 후보로 표시되는지 확인합니다.

---

## AI Workspace Action Feedback Loop — 추천실행목록 현장 답변 기록 (2026-05-13)

### 1. Summary

- 사용자가 `AI 추천 실행 목록` 카드에서 바로 현장 답변을 입력하고 `기록하고 AI 판단`을 실행할 수 있게 했습니다.
- 답변은 신규 `AIWorkspaceActionFeedback` 모델에 저장되고, 관련 고객이 있으면 `History(action_type='memo')` 영업노트도 함께 생성됩니다.
- AI 판단 또는 fallback 규칙이 `종료/목록 제외`로 판단한 액션은 다음 조회부터 추천 실행 목록에서 숨깁니다.
- 기존 짧은 버튼명 `질문/메일/노트/고객/AI`는 `질문 초안`, `메일 초안`, `고객 보기`, `AI 분석`처럼 의미가 드러나게 바꿨습니다.

### 2. Files Changed

| 파일 | 변경 내용 |
| ---- | --------- |
| `AGENT_PLAN.md` | 작업 범위, DB 변경 필요 여부, 검증 계획 기록 |
| `reporting/models.py` | `AIWorkspaceActionFeedback` 모델 추가 |
| `reporting/migrations/0100_aiworkspaceactionfeedback.py` | 신규 feedback 저장 테이블 migration |
| `reporting/admin.py` | feedback 모델 admin 조회 추가 |
| `reporting/views.py` | feedback 저장 API, AI/fallback 판단, 큐 숨김, CRM 메모 생성 |
| `reporting/urls.py` | `/reporting/api/ai-workspace/actions/feedback/` 추가 |
| `reporting/tests.py` | 권한, 저장, 숨김, next action 유지 회귀 테스트 추가 |
| `frontend/src/api.ts` | feedback 타입과 API client 추가 |
| `frontend/src/App.tsx` | 액션 카드 답변 입력/저장 UI, 명확한 버튼 라벨 적용 |
| `frontend/src/styles.css` | feedback 입력/상태 UI 스타일 추가 |

### 3. CRM Improvements

- 추천 실행 목록이 단순 초안 생성 목록이 아니라 현장 답변을 축적하는 작업 큐로 바뀌었습니다.
- 예: `고객이 아직 안산대요`를 기록하면 CRM 메모로 남고, 해당 추천은 종료 처리되어 반복 노출을 줄입니다.
- `추가 자료 요청`, `다음달 검토`처럼 후속이 필요한 답변은 다음 액션/예정일이 있는 메모로 남고 추천 목록에 feedback 요약이 표시됩니다.
- OpenAI가 없거나 실패해도 로컬 규칙으로 `종료` 또는 `다음 액션`을 판단합니다.

### 4. Existing Functionality Preserved

- 기존 `/reporting/*`, React `/ai-workspace/`, AI 허브, PainPoint 검증, 초안 생성 API는 유지했습니다.
- `can_use_ai=False` 사용자는 feedback API도 사용할 수 없습니다.
- CSRF/session 보호를 유지했습니다.
- 초안 생성은 계속 비저장 동작이며, 새 feedback API만 CRM 메모를 생성합니다.
...
→ b516e11 feat: add AI workspace action center
→ d950daa docs: record quote import guard deployment
...
python manage.py makemigrations reporting
→ reporting/migrations/0100_aiworkspaceactionfeedback.py created
...
cd frontend && npm run build
→ OK, dist/assets/index-BWBc_Eg5.js / dist/assets/index-uDvKztaN.css generated
→ Vite chunk size warning only
...
git commit -m "feat: record AI workspace action feedback"
→ e93534d

git push origin main
→ main updated to e93534d
```

### 6. Local Smoke

- Browser plugin의 Node REPL 실행 도구가 노출되지 않아 Playwright CLI로 대체했습니다.
- `http://127.0.0.1:5173/ai-workspace/`를 열었고, 비로그인 상태에서 `/reporting/api/ai-workspace/`가 401을 반환한 뒤 `/reporting/login/?next=%2Fai-workspace%2F`로 이동하는 것을 확인했습니다.
- 로컬 콘솔에는 expected `favicon.ico` 404와 비로그인 API 401만 관찰됐습니다.

### 7. Production Deployment Status

- Implementation commit: `e93534d feat: record AI workspace action feedback`
- GitHub push: `main` updated to `e93534d`
- Railway `web`: `16da0078-61e5-42da-9158-a27358fdd48a` SUCCESS, commit `e93534d`
- Railway `sales-note-frontend`: `323d2236-21f4-485f-b7d1-65b36e9a220e` SUCCESS, 배포 메시지 `Deploy AI workspace feedback e93534d`
- Railway service status: `web`, `sales-note-frontend`, and `Postgres` Online.
- Railway `web` logs checked:
  - `Applying reporting.0100_aiworkspaceactionfeedback... OK`
  - gunicorn started.
...
- Production frontend smoke:
  - `GET https://sales-note-frontend-production.up.railway.app/ai-workspace/` returned 200.
  - Served `/assets/index-BWBc_Eg5.js` and `/assets/index-uDvKztaN.css`.
  - JS bundle contains `기록하고 AI 판단`, `/reporting/api/ai-workspace/actions/feedback/`, `고객 보기`, `질문 초안`.
  - CSS bundle contains `ai-action-feedback-form` and `ai-action-feedback-submit`.
- Production API auth smoke:
  ...
  - Unauthenticated POST to feedback API returned CSRF 403, confirming CSRF protection is active before write handling.

### 8. Known Limitations

- 운영 로그인 세션이 없어 실제 운영 데이터가 들어간 액션 카드 저장 흐름은 자동 검수하지 못했습니다.
- PainPoint 액션에 답변을 남겨도 기존 PainPoint 검증 상태를 자동으로 `verified`로 바꾸지는 않습니다. 이번 작업은 추천실행목록 feedback 루프와 CRM 메모 기록에 한정했습니다.
- OpenAI 실패 시 fallback 규칙은 키워드 기반입니다. 실제 영업 표현이 쌓이면 판단 규칙을 더 정교화할 수 있습니다.

### 9. Recommended Next Task

수동검수 후 M5: feedback 이력/성과 뷰. 추천 액션별 `기록됨`, `종료됨`, `다음 액션 전환됨`을 관리자/본인이 조회하고, AI 추천이 실제 영업노트와 후속조치로 얼마나 이어졌는지 보는 화면을 추가하는 것이 적절합니다.

### 10. Manual Server Test Process

1. 운영 프론트 접속: `https://sales-note-frontend-production.up.railway.app/ai-workspace/`
2. 로그인 후 `AI 추천 실행 목록`에서 액션 카드 하나를 고릅니다.
3. `현장 답변 기록`에 `고객이 아직 안산대요`처럼 구매하지 않는 답변을 입력하고 `기록하고 AI 판단`을 누릅니다.
4. 성공 메시지가 뜨고 해당 액션이 목록에서 사라지는지 확인합니다.
5. 해당 고객 상세 또는 영업노트 목록에서 `[AI 추천 실행 답변]` 메모가 생성됐는지 확인합니다.
6. 다른 추천 액션에는 `고객이 추가 자료를 메일로 보내달래요`처럼 후속이 필요한 답변을 입력합니다.
...
## Next Worker Handoff — AI Situation Sync / CRM State Reconciliation (2026-05-13)

### 1. User Request

사용자는 다음 작업으로 `AI Situation Sync / CRM State Reconciliation`을 시작하길 원합니다. 오늘 컴퓨터를 끌 예정이라 다음 작업자가 이어받을 수 있게 문서에 인수인계를 남겨달라고 했습니다.

핵심 요구:

- 사용자가 AI에게 “이런 상황입니다”라고 알려주면 AI가 메모만 남기지 말고 CRM 상태 전체를 바꿔야 합니다.
- 예: “홍철화 연구원은 견적을 줬지만 아직 안산대요”라고 말했는데도 팔로우업/추천실행목록/알림에 계속 후속조치로 남아 있으면 안 됩니다.
- 부정적이면 기존 FollowUp을 종료/보류하고 추천 목록에서 제거해야 합니다.
- 긍정적이면 오히려 새 FollowUp을 만들거나 다음 연락 일정으로 갱신해야 합니다.
- “메일 보냈는데 아직 답장이 안왔다” 같은 상황도 답장 대기 상태로 잡아내야 합니다.

### 2. Current State

- `AI Workspace Action Feedback Loop`는 이미 배포됐습니다.
  - 사용자는 추천실행목록 카드에서 현장 답변을 저장할 수 있습니다.
  - 답변은 `AIWorkspaceActionFeedback`과 `History(action_type='memo')`로 기록됩니다.
  - `resolved`/`dismissed` 판단은 일부 추천 목록 숨김에 쓰입니다.
- `AI Workspace Feedback Performance View`도 이미 배포됐습니다.
  - AI 지휘석에서 누적 답변/최근 답변/종료/다음 액션 지표를 볼 수 있습니다.
- 아직 부족한 부분:
  - 기존 `FollowUp` 자체를 종료/보류/갱신하지 않습니다.
  - 알림/대시보드/후속조치 화면까지 상태가 일관되게 정리되지는 않습니다.
  - “메일 답장 대기”는 자동 감지나 상태 객체로 아직 반영되지 않습니다.

### 3. Required Next Task

`AGENT_PLAN.md` 맨 아래의 `AI Situation Sync / CRM State Reconciliation — AI 상황 입력 기반 CRM 상태 동기화` 섹션을 먼저 읽고 진행하세요.

이 작업은 기존 권장 작업이던 `M2 Quote Import and Partial Delivery Guards`보다 우선합니다. 단, 시작 전 다음을 확인하세요.
...
1단계는 외부 메일 자동 연동이 아니라, 사용자가 AI 추천실행목록에 입력한 자연어 상황을 기준으로 CRM 상태를 동기화하는 것입니다.

우선 구현할 intent/status:

- `resolved_no_purchase`: 아직 안 삼/구매 의사 없음/실패/보류. 관련 열린 FollowUp 종료 또는 보류, 추천 목록 숨김.
- `follow_up_needed`: 다음 연락/자료 요청/다음달 검토. 기존 FollowUp 갱신 또는 새 FollowUp 생성.
- `positive_buying_signal`: 긍정/구매 의향/승인 예정. 후속조치 유지 또는 우선순위 높은 다음 액션 생성.
- `email_waiting`: 메일 발송 후 답장 대기. 답장 대기 FollowUp 생성 또는 갱신.
- `needs_human_review`: AI 확신 부족. 영업노트만 남기고 자동 상태 변경 없음.

중요한 구현 원칙:

- 삭제하지 말고 종료/보류/갱신으로 처리합니다.
- 같은 고객/견적/action에 열린 FollowUp이 있으면 중복 생성하지 않습니다.
- AI가 닫거나 만든 FollowUp, 생성한 History, 숨긴 추천 액션을 audit 가능하게 남깁니다.
- OpenAI 실패 시에도 키워드 기반 fallback으로 최소 동작해야 합니다.
- 자동 상태 변경은 `can_use_ai` 권한과 기존 session/CSRF 보호를 유지해야 합니다.

### 5. Email Handling Note

사용자는 “AI는 메일도 보잖아요”라고 했지만, 현재 제품 런타임에 Gmail/Microsoft 365 수신함 자동 연동이 붙어 있는지는 확인이 필요합니다.

다음 작업자는 먼저 코드베이스에서 다음을 확인하세요.

- 기존 메일 발송 로그 모델이 있는지
- 수신 메일 저장 모델이 있는지
- Gmail/Microsoft 365 연동 코드가 제품 런타임에 있는지
- 고객 이메일과 메일 로그를 연결할 수 있는지

메일 데이터가 없으면 이번 작업에서 외부 메일 연동을 새로 만들지 말고, 사용자가 “메일 보냈는데 답장 없음”이라고 입력했을 때 `email_waiting` FollowUp/History를 만드는 것부터 구현하세요.

### 6. Suggested Manual Test

1. 운영 또는 로컬 AI 지휘석에서 견적 후속 액션을 선택합니다.
2. `홍철화 연구원은 견적을 줬지만 아직 안산대요`라고 입력합니다.
3. 추천 액션이 사라지고 관련 FollowUp이 종료/보류되는지 확인합니다.
4. 고객 영업노트에 사용자 원문, AI 요약, 적용된 변경사항이 남는지 확인합니다.
5. 새로고침 후 같은 고객/견적이 다시 “해야 할 일”로 뜨지 않는지 확인합니다.
6. 다른 액션에 `관심 있다고 하고 다음주에 다시 연락달래요`라고 입력합니다.
7. 다음주 FollowUp이 생성 또는 갱신되는지 확인합니다.
8. `메일 보냈는데 아직 답장이 안왔어요`라고 입력합니다.
9. 답장 대기 FollowUp/History가 생기고, 이후 미응답 follow-up 추천 후보로 이어질 수 있는지 확인합니다.
