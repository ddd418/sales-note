...
작성일: 2026-07-10
상태: **Phase 0~1 구현·배포 완료 + 전면 쓰기 활성화** (아래 §12 진행 로그)
결정된 방향: **전면 쓰기 프록시** — 인증/권한은 약화하지 않고, 되돌릴 수 없는 액션은 실행 전 확인(428 게이트)으로 보호.
저장소: `D:\projects\sales-note`

> **구현된 권한 모델(2026-07-10, 사용자 결정 반영):** `reporting/write_api.py`는 초기 3개 화이트리스트에서 **allow-all-except-deny + 확인게이트**로 전환됨. `WRITE_DENY_URL_NAMES`(로그인/세션/백업/유저·권한관리/자격증명) 외 모든 POST 쓰기 허용, 되돌릴 수 없는/외부발신 액션(삭제·메일·금액취소·파괴적 대량)은 `X-Salesnote-Write-Confirm: yes` 헤더 없으면 428. 실제 데이터 보호는 acting 유저 `dkswogus95`(id=3, salesman)의 역할/scope(뷰 계층)가 담당.
