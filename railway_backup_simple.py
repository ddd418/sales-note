#!/usr/bin/env python
"""
Railway PostgreSQL 자동 백업 스크립트 (Django 의존성 제거)
매일 오전 8시(한국시간)에 실행되어 데이터베이스를 백업합니다.
...
def create_postgres_dump():
    ...
    try:
        ...
        if not database_url:
            error_msg = "DATABASE_URL 환경변수를 찾을 수 없습니다."
            print(f"❌ {error_msg}")
...
def send_slack_notification(success, message):
    """Slack 알림 전송 (선택사항)"""
    try:
...
"""
        ...
        if success:
            ...
            print(f"🎉 {message}")
            send_slack_notification(True, message)
        else:
            message = "실패했습니다."
            print(f"💥 {message}")
            send_slack_notification(False, message)
            sys.exit(1)
...
"""
        send_email_notification(subject, body, is_success=False)
        send_slack_notification(False, error_message)
        sys.exit(1)
