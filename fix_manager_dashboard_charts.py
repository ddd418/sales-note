...
# -*- coding: utf-8 -*-
"""
매니저 대시보드 차트에 History DeliveryItem 포함 수정 스크립트
"""
...
def fix_customer_distribution():
    ...
    # Line 4363부터 시작하는 3️⃣ 섹션 찾기 (0-indexed: 4362)
    start_idx = None
    end_idx = None
    
    for i, line in enumerate(lines):
        if '# 3️⃣ 고객사별 매출 비중 (Top 5 + 기타) - Schedule 기준' in line and i > 4300:
            start_idx = i
    ...
    if start_idx is None:
        print("❌ 3️⃣ 섹션을 찾을 수 없습니다.")
        return False
    
    print(f"✅ 3️⃣ 섹션 찾음: lines {start_idx+1} ~ {end_idx}")
...
'''
    ...
    print(f"✅ 3️⃣ 섹션 수정 완료")
    return True


def fix_customer_type_stats():
    """6️⃣ 고객 유형별 통계 수정"""
    with open('reporting/views.py', 'r', encoding='utf-8') as f:
        lines = f.readlines()
    
    # Line 4470 근처에서 6️⃣ 섹션 찾기
    start_idx = None
    end_idx = None
    
    for i, line in enumerate(lines):
        if '# 6️⃣ 고객 유형별 통계 (대학/기업/관공서) - Schedule 기준' in line and i > 4400:
            start_idx = i
    ...
    if start_idx is None:
        print("❌ 6️⃣ 섹션을 찾을 수 없습니다.")
        return False
    
    print(f"✅ 6️⃣ 섹션 찾음: lines {start_idx+1} ~ {end_idx}")
...
'''
    ...
    print(f"✅ 6️⃣ 섹션 수정 완료")
    return True


if __name__ == '__main__':
    print("=" * 60)
    print("매니저 대시보드 차트 수정 시작")
    print("=" * 60)
    
    if fix_customer_distribution():
        print("\n3️⃣ 고객사별 매출 비중: Schedule + History 포함 ✅")
    
    if fix_customer_type_stats():
        print("\n6️⃣ 고객 유형별 통계: Schedule + History 포함 ✅")
    
    print("\n" + "=" * 60)
    print("✅ 매니저 대시보드 차트 수정 완료!")
    print("=" * 60)
