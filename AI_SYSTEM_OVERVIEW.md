...
## 🚀 2024.12 AI 시스템 고도화 (GPT 보고서 기반)

### 주요 개선사항

1. **Next-Best-Action 모델 적용**: 추상적 조언 → 구체적 타이밍/채널/대화주제 제안
2. **숨은 니즈 추론**: 고객이 직접 말하지 않은 제약조건/의도 파악
3. **템플릿화 방지**: AI 티 나는 뻔한 표현 금지, 개인화 강화
4. **데이터 기반 일관성**: 등급 평가 시 정량 기준 적용
5. **토큰 효율화**: 히스토리 압축, 필수 정보만 전달
...
```
너는 B2B 영업 20년차의 이메일 작성 전문가

🚨 절대 금지: 템플릿처럼 보이는 이메일
- "안녕하세요, 좋은 하루 보내고 계신가요?" 같은 뻔한 인사 금지
- 반드시 이 고객만의 히스토리에서 1-2가지 구체적 내용을 언급
...
### 3. 고객 인사이트/요약 (`generate_customer_summary`) ⭐ 대폭 개선

| 항목            | 내용                                                    |
| --------------- | ------------------------------------------------------- |
| **목적**        | 고객 정보 종합 분석 및 **실전형 영업 전략** 리포트 생성 |
...
```
당신은 B2B 과학실험실 장비 영업 분석 전문가이자 **실전형 세일즈 코치**입니다.

🚨 핵심 원칙: AI 티 나는 뻔한 조언은 금지!
- "정기적으로 연락하세요" 같은 원론적 조언 금지
- 반드시 이 고객만의 데이터에서 도출한 구체적인 액션을 제시

리포트 구성:
1. 고객 개요 + 의사결정 구조 파악
2. 최근 활동 요약 + 구매 온도 측정 (0-10점)
3. 🔍 숨은 니즈 및 제한조건 추론 (NEW!)
   - "검토해보겠습니다" = 예산 확인 필요
   - 시간적 제약, 내부 의사결정 구조 파악
4. 실험/연구 분야 분석 및 제품 추천
5. 구매 가능성 평가
6. 🎯 Next-Best-Action (NEW!)
   - 언제: 구체적 날짜/시기
   - 어떤 채널로: 전화/이메일/방문/카톡
   - 무슨 주제로: 히스토리 기반 대화 주제
   - 왜 지금인지: 타이밍의 근거
7. 영업 캘린더 제안
```
